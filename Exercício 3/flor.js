...
function main1() {
    ...
    function animate() {
        ...
        const pulseOscillation = 0.08 * Math.sin(time);
        const basePetalScale = 0.4;
        const pulseScale = basePetalScale + pulseOscillation;
        const basePosition = 0.45; 
        const dynamicPosition = basePosition + pulseOscillation;
        const baseCenterScale = 0.9;
        const pulseCenterScale = baseCenterScale - (pulseOscillation * 0.5); 
        ...
        drawObject(pulseCenterScale, 0.0, 0.0, azul);  

        drawObject(pulseScale, dynamicPosition, 0.0, verde); 
        drawObject(pulseScale, -dynamicPosition, 0.0, verde); 
        drawObject(pulseScale, 0.0, dynamicPosition, verde);  
        drawObject(pulseScale, 0.0, -dynamicPosition, verde); 
